<form action="preview.php" class="affix" method="POST" >
	<textarea name="inilah" class="nulis form-control" rows="25" >Pada hari minggu ku turut ayah ke kota, naik __delman__ istimewa ku duduk di muka, ku duduk di samping pak kusir yang sedang _bekerja _  
```javascript
<script type="text/javascript">
  alert("yeah!");
</script>
```
tapi pada kenyataannya adalah ketika kita menulis seperti ini, ada beberapa orang yang tidak suka looh, misalnya :  
1. Tukang humor;
2. Tukang dagang;
3. Tukang kanjut kundang;  
___
Bukan hanya itu, ternyata si jangkar anaknya si jangkur itu adalah penjahat kelas `kakap` dan orang-orang pada nganjuk sama Si kamuu, iiyaaa kamuuu...  
Berikut adalah data orang-orang yang ngehutang sama aku.

| No. | Nama | Hutang |
|:-----|:-------:|----------:|
| 1 | Si jangkar tukang baso tahu| Rp. 25.000.000|
| 2 | Si Bajak tukang bajak langit| Rp. 50.000.000|

Nah, itulah orang-orang yang belum bayar hutang sama saya, selain itu, ada juga si jangkar yang lagi pusing liatin matahari karena matanya gak kuateun :
- Si Koplak
- Si Tidak Koplak, dan
- Si Kaplok.  

Oke mungkin segitu dulu ya tutorial dari saya, semoga bermanfaat
>Sebuah masalah yang telah jelas digambarkan berarti telah terselesaikan sebagian.  
>(C. F. Kettering)

___
[Sumber](http://squhart.web.id)
</textarea>	
</form>


<div class="row pinguin">
  
    <!-- Grid bagian kiri | Kontainer utama -->
    
  <div class="left-container">
    <div class="left-modul thread-modul">
      <h4 class="title">Title</h4>
      <span class="date"><i class="glyphicon glyphicon-time"></i> &nbsp;28 Februari, 2014 &nbsp; &bull; &nbsp;</span>
      <span class="comment"><i class="glyphicon glyphicon-comment"></i> 13 &nbsp;&bull;</span>
      <span class="comment"> &nbsp;Posted by admin</span>
      <br><hr>
      <p class="wtsay diparse"></p>


    </div>
</div>